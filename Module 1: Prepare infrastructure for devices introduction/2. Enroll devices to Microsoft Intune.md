...
- **MDM Authority**: Intune must be set as the Mobile Device Management authority. `📍Path: *Intune Admin Center → Tenant Administration → MDM Authority*`
...
- **Configure Microsoft Entra ID device settings** control who can join devices, enable hybrid join, set max devices per user
  - `Microsoft Entra admin center → Devices → Device settings`
- **Define enrollment restrictions** limit platforms, OS versions, and number of devices per user 
  - `Intune admin center → Devices → Enrollment → Enrollment restrictions`
- **Define enrollment managers** - delegate enrollment permissions for shared or kiosk devices  
  - `Intune admin center → Devices → Enrollment → Enrollment managers`
- **Configure device categories** - tag devices for policy targeting during enrollment 
  - `Intune admin center → Devices → Enrollment → Device categories`
- **Define terms and conditions** - require user acceptance during enrollment
  - `Intune admin center → Devices → Enrollment → Terms and conditions`
- **Configure Entra ID company branding** - customize sign-in and enrollment screens with logos, background images, and help links  
  - `Microsoft Entra admin center → Identity → Branding → Company branding`
- **Enable Multi-Factor Authentication (MFA)** - adds a second layer of identity verification during sign-in and enrollment
  - `Microsoft Entra admin center → Protection → Conditional Access → Named policy (e.g., "Require MFA for all users")`
- **Configure Conditional Access policies** - restrict access to apps/resources based on device compliance, location, risk, or sign-in behavior 
...
- **Create device compliance policies** - define rules for OS version, encryption, antivirus, password, etc.
  - `Intune admin center → Endpoint security → Compliance policies`
