...
### 1. Device Settings → Entra (Azure AD)
...
## 4. Device Enrollment → Intune

- **Automatic MDM Enrollment** → Enable for users or groups  
- **Windows Autopilot Deployment Profiles** → For new or pre-provisioned Windows devices  
- **Enrollment Restrictions** → Control allowed platforms and OS versions  
- **Device Categories** → Tag devices for policy targeting (e.g., IT, HR, Sales)

---

- **Configure Entra ID company branding** - customize sign-in and enrollment screens with logos, background images, and help links `Microsoft Entra admin center → Identity → Branding → Company branding`
- **Create device compliance policies** - define rules for OS version, encryption, antivirus, password, etc. `Intune admin center → Endpoint security → Compliance policies`
- **Enable Multi-Factor Authentication (MFA)** - adds a second layer of identity verification during sign-in and enrollment `Microsoft Entra admin center → Protection → Conditional Access → Named policy (e.g., "Require MFA for all users")`
- **Configure Conditional Access policies** - restrict access to apps/resources based on device compliance, location, risk, or sign-in behavior. `Microsoft Entra admin center → Protection → Conditional Access`
